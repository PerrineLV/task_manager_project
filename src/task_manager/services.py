...
class EmailService:
    ...
    def send_task_reminder(self, email, task_title, due_date):
        ...
        try:
            ...
            with smtplib.SMTP(self.smtp_server, self.port) as server:
                ...
                print(
                    f"Envoi d'un rappel à {email} pour la tâche '{task_title}' - "
                    f"Échéance: {due_date}"
                )
    ...
    def send_completion_notification(self, email, task_title):
        ...
        try:
            ...
            body = (
                f"Félicitations !\n\nVotre tâche '{task_title}' a été marquée "
                f"comme terminée.\n\nCordialement,\nTask Manager"
            )
            msg.attach(MIMEText(body, "plain"))
            ...
            with smtplib.SMTP(self.smtp_server, self.port) as server:
                ...
                # Ici on simule juste l'envoi pour les tests
                print(
                    f"Envoi d'une notification de fin à {email} pour la tâche "
                    f"'{task_title}'"
                )
...
class ReportService:
    ...
    def generate_daily_report(self, tasks, date=None):
        ...
        completed_tasks = sum(
            1 for task in daily_tasks if hasattr(task, "completed") and task.completed
        )
    ...
    def export_tasks_csv(self, tasks, filename):
        ...
        try:
            ...
            raise PermissionError(
                f"Permission refusée pour écrire le fichier: {filename}"
            )
        except IOError as e:
